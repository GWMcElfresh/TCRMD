...
which converts Docker images into portable, read-only Singularity Image Format
(`.sif`) files.  Each TCRMD Docker image becomes a self-contained Apptainer
image that includes all conda dependencies.

---

## Step 1 — Build Apptainer images from Docker

Perform these steps **on a machine where you have Docker and root (or rootless
Docker) access**, then transfer the `.sif` files to your HPC.
...
Transfer the `.sif` files and your data to the cluster:

```bash
scp tcrmd-*.sif user@hpc.example.edu:/scratch/user/tcrmd/
scp -r tests/   user@hpc.example.edu:/scratch/user/tcrmd/
```

> **Tip:** If your HPC has internet access, you can build the images directly
> on the login node using `apptainer build --fakeroot`.  Check your cluster's
